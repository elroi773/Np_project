...
export default {
  ...
  mounted() {
    ...
    if (window.kakao && window.kakao.maps) {
      window.kakao.maps.load(() => {
        ...
        let allStores = JSON.parse(localStorage.getItem("store_data")) || [];
        allStores.push(store);
        localStorage.setItem("store_data", JSON.stringify(allStores));
  ...
  methods: {
    ...
    goNext() {
      this.$router.push({ name: 'Join4Sell' });
    }
  },
};
</script>
